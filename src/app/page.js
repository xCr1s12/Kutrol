...
export default function Home() {
  ...
  return (
    <div className={`w-full min-h-screen bg-[#1B2B24] overflow-hidden ${big_Shoulders.variable}`} lang="es">
      ...
      <header className="w-full bg-[#1B2B24] flex items-center justify-between px-4 sm:px-8 py-3 sm:py-4 border-b-[#314139] border-b-2 flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          <button onClick={()=>{handleRedirect("/login")}} className="text-xs sm:text-sm font-medium text-[#A8B0AB] transition-colors hover:opacity-80 px-2 sm:px-4 py-1 sm:py-2">
              Iniciar Sesion
          </button>

          <button onClick={()=> {handleRedirect("/register")}} className="rounded-md bg-[#D9AC34] px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm font-semibold text-white transition-opacity hover:opacity-90">
            Registrarse
          </button>
        </div>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 w-full min-h-fit lg:min-h-screen items-center px-6 sm:px-8 lg:px-18 gap-8 lg:gap-12 py-12 lg:py-0">
        ...
        <div className="flex flex-col gap-4 items-center text-center lg:items-start lg:text-left">
          ...
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold uppercase leading-tight text-[#FDFBF7]">
            Rentabilidad <br />
            Para ti <br />
            Respiro para el <br />
            planeta
     ...
     {/* Seccion  "Que hace Kutrol" */}
      <section className="relative w-full px-6 md:px-16 py-12 md:py-16 md:pl-32">
        ...
        <div className="mb-10 text-center md:text-left">
          <p className="text-sm text-[#A8B0AB] font-semibold"> Que Hace</p>
          <h1 className="font-bold text-2xl md:text-4xl text-[#FDFBF7]"> Que puedes hacer con Kutrol</h1>
        </div>
        ...
        <div className="max-w-4xl mx-auto md:mr-0 md:ml-auto flex flex-col gap-y-8 md:gap-y-12">
          <div className="flex flex-col gap-y-4 pb-8 border-b-2 border-[#314139]"> 
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-y-3 sm:gap-x-8">
              ...
              <div className="flex flex-col gap-y-1">
                <h3 className="text-lg sm:text-2xl font-bold">Registra tus Vehículos</h3>
                <p className="text-sm text-[#A8B0AB]">Suma cada unidad de tu flota con sus datos básicos y documentos.</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-y-4 pb-8 border-b-2 border-[#314139]">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-y-3 sm:gap-x-8">
              ...
              <div className="flex flex-col gap-y-1">
                <h3 className="text-lg sm:text-2xl font-bold">Registra Tus Datos</h3>
                <p className="text-sm text-[#A8B0AB]">Cada carga de combustible y cada viaje queda guardado automáticamente.</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-y-4 pb-8 border-b-2 border-[#314139]">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-y-3 sm:gap-x-8">
              ...
              <div className="flex flex-col gap-y-1">
                <h3 className="text-lg sm:text-2xl font-bold">Vizualiza y Optimiza</h3>
                <p className="text-sm text-[#A8B0AB]">El panel te muestra dónde estás gastando de más y qué vehículos necesitan atención.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="relative w-full flex items-center justify-center px-6 md:px-16 py-12 md:py-16 md:pl-32 bg-[#1B2B24] text-[#FDFBF7]">
        ...
        <div className="max-w-6xl w-full">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between rounded-2xl bg-[#314139] p-6 sm:p-10 gap-y-4 sm:gap-y-0">
            ...
            <button onClick={()=> {handleRedirect("/register")}} className="rounded-lg bg-[#D4AF37] px-6 py-3 text-sm font-semibold text-[#FDFBF7] transition-opacity hover:opacity-90 w-full sm:w-auto shrink-0 text-center">
              Registrarse
            </button>
          </div>
        </div>
      ...
      <footer className="w-full flex flex-col mt-5 gap-1 pb-4">
        ...
        <p className="text-xs sm:text-sm text-[#A8B0AB] px-5 text-center sm:text-left"> Proyecto de Ingenieria Civil en Informatica | Ulagos 2026 </p>
      </footer>
    </div>  
  );
}